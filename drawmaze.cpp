...
namespace DrawMaze {
    ...
    CellParams get_cell_params(int size, int maze_radius, MazeParams maze_params, Cell* cell) {
    ...
    PNG draw_maze(const Board& maze, int size, string colour_mode) {
        ...
        for (const auto& kv : maze.cells) {
            ...
            draw_agon(img, cell_params, cell_colour, cell_colour);
        }
        ...
        if (colour_mode == "none") {
            ...
            Cell* start_cell = maze.cells.at(maze.start_coord);
            CellParams cell_params = get_cell_params(size, maze.radius, params, start_cell);
            draw_agon(img, cell_params, start_colour, start_colour);

            Cell* end_cell = maze.cells.at(maze.end_coord);
            cell_params = get_cell_params(size, maze.radius, params, end_cell);
            draw_agon(img, cell_params, end_colour, end_colour);
        }
    ...
    PNG draw_solution(const Board& maze, int size, string colour_mode) {
        ...
        for (int i = 0; i < sol_length - 1; i++) {
            ...
            Cell* cell = maze.cells.at(maze.solution[i]);
            CellParams cell_params = get_cell_params(size, maze.radius, params, cell);

            // get centre coorindates of next cell
            Cell* next_cell = maze.cells.at(maze.solution[i + 1]);
    ...
    void draw_agon(PNG& img, const CellParams& cell_params, const RGBAPixel& edge_colour, const RGBAPixel& fill_colour) {
        // get the points on the edges of the agon
